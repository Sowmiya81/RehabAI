...
# Secure version with automatic temp file cleanup.
# """
...
# def cleanup_on_exit():
#     """Delete all temp files when app closes."""
...
#     """Format detected issues as markdown."""
#     if not issues:
...
#             'moderate': '🟡 **MODERATE**',
#             'mild': '🟢 **MILD**'
#         }.get(issue.get('severity', 'mild'), '⚪ **MILD**')
        
#         issue_title = issue['type'].replace('_', ' ').title()
#         md += f"### {issue_title} {severity_emoji}\n\n"
...
#     """Format biomechanics metrics as markdown table."""
#     quality = biomech_data.get('quality_score', 'N/A')
#     reps = biomech_data.get('rep_count', 'N/A')
...
#         md += "| Measurement | Left | Right | Difference |\n"
#         md += "|------------|------|-------|------------|\n"
...
#                     diff = abs(left - right)
#                     md += f"| {metric_display} | {left:.1f}° | {right:.1f}° | **{diff:.1f}°** |\n"
#                 else:
#                     md += f"| {metric_display} | {left} | {right} | - |\n"
    
#     return md


# def format_evidence(evidence_chunks):
#     """Format evidence chunks as markdown."""
#     if not evidence_chunks:
#         return "_No research evidence retrieved_"
    
#     md = f"📚 **Retrieved {len(evidence_chunks)} research sources**\n\n"
    
#     for i, chunk in enumerate(evidence_chunks[:3], 1):
#         score = chunk.get('relevance_score', 0)
#         text = chunk.get('text', '')[:250]
        
#         md += f"**Source {i}** (Relevance: {score:.2f})\n\n"
#         md += f"> {text}...\n\n"
#         md += "---\n\n"
...
#     """Enhanced analysis with automatic temp file cleanup."""
#     if video_file is None:
#         return (
#             "⬆️ Upload a video to begin analysis",
#             "", "", "", "⚠️ Please upload a video file"
#         )
    
#     temp_file_path = video_file  # Track for cleanup
    
#     try:
#         progress(0.1, desc="Loading video...")
#         print(f"Processing video: {video_file}")
        
#         progress(0.3, desc="Analyzing movement...")
#         result = agent.run(video_file)
        
#         progress(0.8, desc="Generating coaching...")
...
#         issues_md = format_issues(issues)
#         metrics_md = format_metrics(biomech)
#         evidence_md = format_evidence(evidence)
        
#         progress(1.0, desc="Complete!")
        
#         status = f"✅ **Analysis complete**\n\n🎯 Detected **{len(issues)}** issues | 📚 Used **{len(evidence)}** research sources"
        
#         print(status)
        
#         return issues_md, metrics_md, coaching, evidence_md, status
...
# with gr.Blocks(theme=gr.themes.Soft(), title="RehabAI") as demo:
    
#     gr.Markdown("""
#     # 🏋️ RehabAI - AI Movement Analysis Coach
    
#     Upload a squat video filmed from the **FRONT** to receive evidence-based coaching powered by computer vision and AI.
    
#     **📹 Camera Guidelines:**
#     - Record from the front (not side angle)
#     - Full body visible (head to feet)
#     - Good lighting
#     - 6-10 feet distance from camera
#     """)
...
#         "⬆️ Upload a video and click **'Analyze Movement'** to begin"
#     )
...
#     with gr.Tabs():
#         with gr.Tab("🎯 Biomechanics Analysis"):
#             with gr.Row():
#                 with gr.Column(scale=1):
#                     issues_output = gr.Markdown(
#                         "Detected issues will appear here after analysis..."
#                     )
                
#                 with gr.Column(scale=1):
#                     metrics_output = gr.Markdown(
#                         "Biomechanics metrics will appear here after analysis..."
#                     )
        
#         with gr.Tab("📝 Coaching Plan"):
#             coaching_output = gr.Markdown(
#                 "Your personalized coaching plan will appear here after analysis..."
#             )
        
#         with gr.Tab("📚 Research Evidence"):
#             evidence_output = gr.Markdown(
#                 "Research sources supporting the recommendations will appear here..."
#             )
...
#     **Technology Stack:**
#     - 🎥 MediaPipe Pose for biomechanics analysis
#     - 📚 RAG (Retrieval-Augmented Generation) for evidence-based recommendations
#     - 🤖 Gemini 2.5 Flash for AI coaching generation
    
#     **Privacy & Security:**
#     - 🔒 Videos processed locally on your computer
#     - 🔒 Temporary files deleted immediately after analysis
#     - 🔒 No data stored or uploaded to cloud
...
#         server_port=7860,
#         share=False,              # ✅ No internet sharing
#         show_error=True
#     )
...
"""
RehabAI - AI Movement Analysis Coach
Professional Gradio interface with polished design.
"""
